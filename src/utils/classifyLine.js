...
export function classifyLine(item, requestedQty, requestedRate, proposedBrand, proposedModel, tolerancePct = 5, secondApprovalPct = 15) {
  if (!item) {
  ...
  const remainingQty = (item.qty || 0) - (item.committedQty || 0);
  ...
  if (requestedQty > remainingQty) { lane = "exception"; reasons.push(`Quantity exceeds remaining approved balance by ${fmtNum(requestedQty - remainingQty)} ${item.unit || "Nos"}`); }
